...
const DEPARTMENTS = ["Mechanic", "HR", "Design", "Electrical", "Accounting"];
...
function classifyStatus(
  ...
  if (timeLog?.status) {
    ...
    if (s === "on_time" || s === "late" || s === "absent" || s === "leave")
      return s;
  }
...
function StatCard({
  ...
  return (
    <div
      className={`relative bg-white rounded-2xl border shadow-sm p-4 flex items-center gap-4 overflow-hidden ${comingSoon ? "border-gray-100 opacity-75" : "border-gray-100"}`}
    >
      ...
      <div
        className={`w-12 h-12 rounded-2xl flex items-center justify-center flex-shrink-0 ${accent}`}
      >
        {icon}
      </div>
      <div className="min-w-0">
        ...
        <p
          className={`text-2xl font-extrabold leading-tight ${comingSoon ? "text-gray-300" : "text-gray-800"}`}
        >
          {value}
        </p>
        {sub && (
          <p
            className={`text-xs mt-0.5 ${comingSoon ? "text-gray-300" : "text-gray-400"}`}
          >
            {sub}
          </p>
        )}
    ...
    <div>
      ...
      <div className="grid grid-cols-7 gap-1">
        {cells.map((day, idx) => {
          ...
          const isOffWeekend =
            (isSunday || isSat) && (!log || log.status === "holiday");
          const isWork =
            log &&
            log.checkIn !== null &&
            log.workType !== "holiday" &&
            log.status !== "leave" &&
            !isOffWeekend;
        ...
        {loading ? (
          ...
          <>
            {balances.map((b) => {
              ...
              if (b.leave_type === "other" && Number(b.used_days) === 0)
                return null;
                ...
                <div key={b.id}>
                  <div className="flex justify-between items-center mb-1.5">
                    <div className="flex items-center gap-2">
                      <span
                        className={`text-xs font-bold ${cfg?.color ?? "text-gray-600"}`}
                      >
                        {b.label_th}
                      </span>
                    </div>
...
export default function ProfilePage() {
    ...
    try {
      const [timeRes, holidayRes, reportRes, otReqRes] = await Promise.all([
        ...
        supabase
          ...
          .select(
            "log_date, work_type, first_check_in, last_check_out, ot_hours, status, timeline_events, daily_allowance",
          )
          .eq("user_id", userId)
          .gte("log_date", start)
          .lte("log_date", end),
      ...
      const reportSet = new Set<string>(
        (reportRes.data ?? []).map((r) => r.report_date),
      );
  ...
  const reportTotal = useMemo(
    () =>
      logs.filter(
        (l) =>
          l.checkIn !== null && l.status !== "leave" && l.status !== "holiday",
      ),
    [logs],
  );
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 pt-5 space-y-5">
              ...
              {profileLoading ? (
                ...
                <div className="mt-2">
                  ...
                  <p className="text-sm text-gray-400 mt-0.5">
                    {profileForm.department || "ยังไม่ได้ระบุแผนก"} ·{" "}
                    {userEmail}
                  </p>
                </div>
              )}
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4">
          <div className="grid grid-cols-4 divide-x divide-gray-100">
            ...
            <div className="text-center px-2">
              <p
                className={`text-lg font-extrabold ${lateCount === 0 ? "text-emerald-500" : "text-amber-500"}`}
              >
